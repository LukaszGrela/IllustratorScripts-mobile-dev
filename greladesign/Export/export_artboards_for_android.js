...
(function() {
  if (app.documents.length == 0) {
    alert(
      'Open a document containing some artboards to export before running this script'
    );
    return;
  }
  var extensionFinder = /(\.[^\.]*)$/;
  ...
  var saveInBucket = function(folder, artboard) {
    ...
    var myFolder = new Folder(
      folder.absoluteURI + '/' + documentName + '/drawable-' + bucket
    );
  ...
  var i, ab, folder, pattern;
  pattern = prompt(
    'Give pattern of artboard names to export, leave asterisk (*) to include all (case insensitive).',
    '*'
  );
  ...
  for (i = artboards.length - 1; i >= 0; i--) {
    ...
    ab = artboards[i];
    if (pattern === '*' || ab.name.toLowerCase().indexOf(pattern) !== -1) {
